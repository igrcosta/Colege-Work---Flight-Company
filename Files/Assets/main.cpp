...
int CadastroPassageiro(PASSAGEIRO& passageiro) {
    ...
    do {
        ...
        // Gerar código do passageiro
        int codigoPassageiro = contadorCodigoPassageiro++;

        // Salvar dados no arquivo
        arquivo << codigoPassageiro << "," << nomePassageiro << ",Passageiro," << telefonePassageiro << "\n";

        cout << "\nPassageiro cadastrado com sucesso! Código: " << codigoPassageiro << "\n";

        // Perguntar se deseja continuar
        cout << "Deseja cadastrar outro passageiro? (1 - Sim, 0 - Não): ";
        cin >> continuar;
...
int CadastroTripulacao(TRIPULACAO& tripulacao) {
    ...
    do {
        ...
        cout << "Digite o número de telefone do tripulante: ";
        ...
        switch (tipo) {
            ...
            case 3: cargo = "Comissário"; break;
            default:
                cout << "Opção inválida! Cadastro cancelado.\n";
                return -1;
        }

        // Gerar código do tripulante
        int codigoTripulante = contadorCodigoTripulacao++;

        // Salvar dados no arquivo
        arquivo << codigoTripulante << "," << nomeTripulante << "," << cargo << "," << telefoneTripulante << "\n";

        cout << "\nTripulante cadastrado com sucesso! Código: " << codigoTripulante << "\n";

        // Perguntar se deseja continuar
        cout << "Deseja cadastrar outro tripulante? (1 - Sim, 0 - Não): ";
        cin >> continuar;
...
public:
    ...
    static int contadorCodigoVoo = 2000; // Código inicial para voos
    static int contadorCodigoAviao = 100; // Código inicial para aviões

    // Ler contadores existentes dos arquivos, se disponíveis
    ...
    cout << "Código do voo gerado automaticamente: " << codigoVoo << "\n";

    // Gerar código do avião
    int codigoAviao = contadorCodigoAviao++;
    cout << "Código do avião gerado automaticamente: " << codigoAviao << "\n";
...
do {
    ...
    if (cin.fail()) {
        ...
        continue; // Volta ao início do loop para pedir novamente o código
    }

    pilotoValido = validarTripulante(codigoPiloto, "Piloto");

    if (!pilotoValido) {
        cout << "Erro: Código do piloto inválido ou não encontrado! Tente novamente.\n";
    }
} while (!pilotoValido);
...
do {
    ...
    if (!copilotoValido) {
        cout << "Erro: Código do copiloto inválido ou não encontrado! Tente novamente.\n";
    }
...
do {
    ...
    comissarioValido = validarTripulante(codigoComissario, "Comissário");

    if (!comissarioValido) {
        cout << "Erro: Código do comissário inválido ou não encontrado! Tente novamente.\n";
    }
...
bool validarTripulante(int codigo, const string& cargoEsperado) {
    ...
    if (!arquivo.is_open()) {
        cerr << "Erro ao abrir o arquivo de tripulantes para validação.\n";
        return false;
    }
            ...
            if (assento.getStatus() == "livre") {
                ...
                if (tipo == 1) {
                    ...
                    switch (opcaoCargo) {
                        ...
                        case 3: cargo = "Comissário"; break;
                        default:
                            cout << "Opção inválida! Reserva cancelada.\n";
                            return;
                    }
                    tipoPessoa = cargo;
                } else {
                ...
                // Atualiza o assento com as novas informações
...
void pesquisarPessoa() {
    string termoBusca;
    cout << "Digite o nome ou código da pessoa que deseja buscar: ";
    cin.ignore();
    getline(cin, termoBusca);

    // Variáveis para armazenar detalhes da pessoa encontrada
    ...
    // Se não encontrado, verificar nos tripulantes
    if (!encontrado) {
    ...
    cout << "\nDeseja listar os voos dessa pessoa? (1 - Sim, 0 - Não): ";
    int listarVoos;
    cin >> listarVoos;
